...
var tplAst = parse("var a = 'hello world!'");
console.log(tplAst);
*/
var parse = require('esprima').parse,
...
function wrap(ugAst){
	if(ugAst && ugAst.body && ugAst.body.length && ugAst.body[0].expression && ugAst.body[0].expression.arguments){
		var charMap = {};
		var charArr = [];
		var tpl = fs.readFileSync(path.join(__dirname, 'src/template.js'));
...
fromStr.inplace = function (file) {
	...
	out = fromStr(src);
	fs.writeFileSync(file.split('.js')[0].replace(/source/,"libs") + ".min.js", out);
};

fs.readdir('./source', function(err, files){
	//console.log(err);
	for(var i=0,j=files.length;i<j;i++){
		fromStr.inplace('./source/' + files[i]);
	}
});
